...
import com.project.core.utils.web.ServletUtil;
...
public class StaticContentServlet extends HttpServlet {
	...
	/** 需要被Gzip压缩的Mime类型. */
	private static final String[] GZIP_MIME_TYPES = { "text/html", "application/xhtml+xml", "application/octet-stream", "text/plain", "text/css",
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!ServletUtil.checkIfModifiedSince(request, response, contentInfo.lastModified)
				|| !ServletUtil.checkIfNoneMatchEtag(request, response, contentInfo.etag)) {
			return;
		}

		// 设置Etag/过期时间
		ServletUtil.setExpiresHeader(response, ServletUtil.ONE_YEAR_SECONDS);
		ServletUtil.setLastModifiedHeader(response, contentInfo.lastModified);
		ServletUtil.setEtag(response, contentInfo.etag);
		...
		if (request.getParameter("download") != null) {
			ServletUtil.setFileDownloadHeader(response, contentInfo.fileName);
		}
