...
<div class="container-right">
    ...
    <form id="mainForm" action="${ctx}/finance/income/debit" method="post">
        <input type="hidden" name="pageNo" id="pageNo" value="${page.pageNo}"/>
        <input type="hidden" name="settlementTime" value="${settlementTime}"/>
        <input type="hidden" name="channelId" value="${channelId}"/>
        <input type="hidden" name="inn  Id" value="${innId}"/>
    ...
    <table class="kz-table" cellpadding="8">
        ...
        <c:forEach items="${page.result}" var="order">
            <tr>
                ...
                <td>${order.financeSpecialOrder.channelDebit}</td>
                <td>${order.financeParentOrder.extraPrice}</td>
                <td>${order.financeSpecialOrder.innPayment}</td>
                <td>${order.financeSpecialOrder.fqBear}</td>
                <td>${order.financeSpecialOrder.fqIncome}</td>
                <td>
                    <c:choose>
    ...
    <c:if test="${not empty orderStatistic}">
        <p>
            ...
            分销商扣赔付总额<em style="color:red;"> ${orderStatistic.channel_debits } </em> 元，
            客栈赔付总额<em style="color:red;"> ${orderStatistic.inn_payments } </em>元，
            番茄承担总额<em style="color:red;"> ${orderStatistic.fq_bears } </em>元，
            番茄收入总额<em style="color:red;"> ${orderStatistic.fq_incomes } </em>元。
        </p>
    </c:if>
