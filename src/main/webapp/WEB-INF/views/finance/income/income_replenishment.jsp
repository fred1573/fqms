...
<div class="container-right">
    ...
    <form id="mainForm" action="${ctx}/finance/income/replenishment" method="post">
    ...
    <table class="kz-table" cellpadding="8">
        ...
        <c:forEach items="${page.result}" var="order">
            <tr>
                ...
                <td>${order.financeParentOrder.innAmount}</td>
                <td>${order.financeParentOrder.innSettlementAmount}</td>
                <td>${order.financeSpecialOrder.fqReplenishment}</td>
                <td>
                    <c:choose>
    ...
    <c:if test="${not empty orderStatistic}">
        <p>
            ...
            分销商结算总额为<em style="color:red;"> ${orderStatistic.channel_settlement_amounts } </em> 元，
            客栈结算总额为<em style="color:red;"> ${orderStatistic.inn_settlement_amounts } </em>元，
            番茄补款金额为<em style="color:red;"> ${orderStatistic.fq_replenishments } </em>元。
        </p>
    </c:if>
