...
<div class="container-right">
    <form id="mainForm" action="${ctx}/finance/out/special/list" method="post">
        ...
        <input type="hidden" name="status" value="special">
    </form>
    <div class="header">
    ...
    <p>
        <c:if test="${not empty allMap and allMap.orders !=0 and allMap.orders != null}">
            截止您所选时间段内，共有<em style="color:red;">${allMap.inncount}</em>家客栈,共有<em style="color:red;">${allMap.orders}</em>个订单，分销商订单总金额为<em style="color:red;">${allMap.channelamount}</em>,客栈订单总金额为<em style="color:red;">${allMap.total}</em>,分销商实际结算金额为<em style="color:red;">${allMap.channels}</em>，番茄收入金额为<em
            style="color:red;">${allMap.fqs}</em>，客栈实际结算金额为<em style="color:red;">${allMap.after}</em>；
        </c:if>
    </p>
