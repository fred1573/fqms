...
<div class="container-right">
    <form id="mainForm" action="${ctx}/finance/out/specialBalance/replenishment" method="post">
    ...
    <table class="kz-table" cellpadding="8">
        ...
        <c:forEach items="${page.result}" var="specialOrder">
            <tr>
                ...
                <td>${specialOrder.financeParentOrder.channelSettlementAmount}</td>
                <td>${specialOrder.fqReplenishment}</td>
                <td>${specialOrder.financeParentOrder.innSettlementAmount}</td>
    ...
    <p>
        <c:if test="${not empty innOrderCount and innOrderCount.orders !=0}">
            截止您所选时间段内，共有<em style="color:red;"><c:if test="${innOrderCount.orders==null}">0</c:if>${innOrderCount.orders}</em>个订单，分销商结算总额为<em style="color:red;"><c:if test="${innOrderCount.channels==null}">0</c:if>${innOrderCount.channels}</em>，番茄补款金额为<em style="color:red;"><c:if test="${innOrderCount.replenishment==null}">0</c:if>${innOrderCount.replenishment}</em>，客栈结算总额为<em style="color:red;">${innOrderCount.inns}</em>，
        </c:if>
    </p>
