...
<div class="container-right">
    <form id="mainForm" action="${ctx}/finance/order/special/debit" method="post">
        ...
        <c:if test="${not empty page}">
            <c:forEach items="${page.result}" var="order">
                <tr>
                    ...
                    <td>${order.financeParentOrder.totalAmount}/${order.channelDebit}/${order.innPayment}</td>
                    <td>${order.fqBear}/${order.fqIncome}</td>
    ...
    <p>
        <c:if test="${not empty totalOrder and totalOrder.total !=0}">
            共计<em style="color:red;">${totalOrder.total}</em>个订单;分销商订单总额为<em style="color:red;">${totalOrder.channel}</em>元;分销商扣赔付总额<em style="color:red;">${totalOrder.total_channel_debit}</em>元;
            客栈赔付总额<em style="color:red;">${totalOrder.inn_payment}</em>元;番茄承担总额<em style="color:red;">${totalOrder.fq_bear}</em>元;番茄收入总额<em style="color:red;">${totalOrder.fq_income}</em>元。
        </c:if>
    </p>
    <c:if test="${not empty page}">
